...
class InvoiceDueDetail extends StatefulWidget {
  final invoiceObject;

  InvoiceDueDetail({this.invoiceObject});
  @override
  _InvoiceDueDetailState createState() => _InvoiceDueDetailState();
}

class _InvoiceDueDetailState extends State<InvoiceDueDetail> {
  ...
  void seenNote() async {
    await dbmanager
        .getSingleNotificationByType(invoiceModel.id.toString(), 'invoice')
        .then((onValue) async {
      if (onValue != null) {
  ...
  void getInvoiceSideInfo() async {
    ...
    var due = DateTime.parse(invoiceModel.invoice_due_date);
    setState(() {
      leftDueDay = due.difference(now).inDays;
    });
                      ...
                      child: Column(
                        children: <Widget>[
                          Padding(
                            ...
                            child: Text(
                              leftDueDay > 0
                                  ? "${getTranslated(context, "note_left_day")}: $leftDueDay"
                                  : getTranslated(context, "note_due_over"),
                              style: TextStyle(
  ...
  Future<pw.Document> generateDocument(PdfPageFormat theFormant) async {
    ...
    try {
      final image = await QrPainter(
        data: invoiceModel.qr_code_string,
